...
2. Fetch `https://cmn.dev/.well-known/cmn.json`
3. Read primary capsule `capsules[0]`
4. Find `type: "mycelium"` endpoint, get its `url` template and `hash` field
5. Replace `{hash}` in the `url` template → fetch primary mycelium manifest. If `hashes` array exists, fetch overflow shards and merge spore lists.
